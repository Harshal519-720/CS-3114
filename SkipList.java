...
public class SkipList<K extends Comparable<? super K>, V> implements Iterable<KVPair<K, V>> {
	 ...
	 * 
	 * @return a random level with minimum value of one
	 */
	 ...
	 * @return a list of KVPairs matching the key
	 */
	...
	public void insert(KVPair<K, V> it) {
		...
		if (newLevel > head.forward.length) {
			adjustHead(newLevel);
		}

		SkipNode[] update = (SkipNode[]) Array.newInstance(SkipNode.class, head.forward.length);
	...
	public void adjustHead(int newLevel) {
		...
		for (int i = 0; i < oldHead.forward.length; i++) {
	...
	public void dump() {
		...
		System.out.println("Node with depth " + current.forward.length + ", Value " + current.element());
	...
	private class SkipNode {
		...
		public SkipNode(KVPair<K, V> tempPair, int level) {
			pair = tempPair;
			forward = (SkipNode[]) Array.newInstance(SkipNode.class, level);
		}
